...
const FileInput: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (typeof defaultValue === "string")
      return setIds([defaultValue.split("/").reverse()[0]]);
  ...
  return (
    <label
      ...
      className={`flex flex-col rounded-md px-4 py-3 border border-gray-light ${
        className ? className : ""
      } ${disabled ? "bg-disabled" : "cursor-pointer"}`}
    >
      {!disabled && placeholder && (
        <p className="text-sm text-gray mb-2">{placeholder}</p>
      )}
      {disabled && !defaultValue && (
        <p className="text-sm text-gray">No file</p>
      )}
      {!defaultValue ? null : (
  ...
  return (
    <span key={id + i} className="mb-3">
      <span className="flex">
        ...
        >
          View {title} {typeof i === "number" ? i + 1 : ""}
        </button>
        {onDelete && (
          <button onClick={onClickDelete} type="button">
            <IoTrashOutline size={16} className="ml-2 text-rose" />
          </button>
        )}
      </span>
