  ...
  const sendOTP = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      toast.loading("Generating OTP...", {
        toastId: "forgot-password-send-otp",
        updateId: 1,
      });
      const response = await fetch(
        `${BASE_URL}/mapping/send_email_otp_forgot_password`,
        {
          method: "POST",
      ...
      setOtpSent(true);
      toast.info(
        "An OTP has been sent to your email. Please enter the OTP and reset your password.",
        { toastId: "forgot-password-send-otp", updateId: 2 }
      );
      return result;
    } catch (e) {
      toast.dismiss("forgot-password-send-otp");
